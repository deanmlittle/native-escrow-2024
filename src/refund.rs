use solana_program::{
    msg, account_info::AccountInfo, entrypoint::ProgramResult, program::invoke_signed, program_error::ProgramError, program_pack::Pack, pubkey::Pubkey
};
...
pub fn process(accounts: &[AccountInfo<'_>]) -> ProgramResult {
    ...
    msg!("Trying to get escrow data");

    msg!("Escrow size = {}", escrow.data_len());


    // Get escrow account data
    let escrow_data: Escrow = *bytemuck::try_from_bytes::<Escrow>(*escrow.data.borrow()).map_err(|e| {
        msg!("{:?}", e);
    ...
    msg!("Invoking to refund vault funds");
    ...
    msg!("Now we just need to close the account");
