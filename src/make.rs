use solana_program::{
    msg,
    account_info::AccountInfo, entrypoint::ProgramResult, program::{invoke, invoke_signed}, program_error::ProgramError, program_pack::Pack, pubkey::Pubkey, rent::Rent, system_instruction::create_account, sysvar::Sysvar
};
...
pub fn process(accounts: &[AccountInfo<'_>], data: &[u8]) -> ProgramResult {
    let Make { seed, amount, receive } = bytemuck::try_pod_read_unaligned::<Make>(&data[..]).map_err(|e| {
        msg!("{:?}", e);
    ...
    // Check PDA of escrow
    let (escrow_address, bump) = Pubkey::try_find_program_address(&[b"escrow", maker.key.as_ref(), seed.to_le_bytes().as_ref()], &crate::ID).ok_or(ProgramError::InvalidAccountData)?;
    ...
    invoke_signed(
        ...
        &[&[b"escrow", maker.key.as_ref(), seed.to_le_bytes().as_ref(), &[bump]]]
    )?;
